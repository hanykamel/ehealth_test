...
{
    public class CreateResourceUHIAPricesCommandValidator : AbstractValidator<CreateResourceUHIAPricesCommand>
    {
        private readonly IResourceUHIARepository _resourceUHIARepository;
        private bool _validResourceUHIA = false;
        public CreateResourceUHIAPricesCommandValidator(IResourceUHIARepository resourceUHIARepository)
        {
            _resourceUHIARepository = resourceUHIARepository;

            RuleFor(x => x.ResourceUHIAId).NotNull().NotEmpty();
            RuleFor(x => x.ResourceItemPrices).NotNull().NotEmpty();
            ...
            RuleFor(x => x.ResourceItemPrices).MustAsync(async (Model, ResourceItemPrices, CancellationToken) =>
            {
                try
                {
                    var resourceUHIA = await ResourceUHIA.Get(Model.ResourceUHIAId, _resourceUHIARepository);

                    foreach (var item in Model.ResourceItemPrices)
                    {
                        if (item.EffectiveDateFrom.Date < resourceUHIA.DataEffectiveDateFrom.Date ||
                         (item.EffectiveDateTo.HasValue && resourceUHIA.DataEffectiveDateTo.HasValue && item.EffectiveDateTo.Value.Date > resourceUHIA.DataEffectiveDateTo.Value.Date) ||
                         ((!item.EffectiveDateTo.HasValue) && resourceUHIA.DataEffectiveDateTo.HasValue))
                        {
                            return false;
                        }
                    }
